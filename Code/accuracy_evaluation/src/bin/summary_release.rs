...
const RESULTSDIR:&str = "./results_2000/";
fn main() -> std::io::Result<()> {
    ...
    // Read every results
    let datasets = ["random", "popular", "mostdep"];
    for dataset in datasets {
        println!("Dataset: {}", dataset);
        let paths = fs::read_dir(RESULTSDIR.to_string() + dataset + "/output/results").unwrap();
        ...
        println!("Resolution Accuracy Summary: ");
