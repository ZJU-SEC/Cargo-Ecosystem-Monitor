...
impl<D: DepOps> DepTreeManager<D> {
    ...
    pub fn new(ops: D) -> Result<Self, AuditError> {
        let lockfile = Lockfile::load("Cargo.lock").map_err(|e| {
            AuditError::InnerError(format!(
                "cannot build DepTreeManager, load lock file fails: {e}",
            ))
        })?;

        let deptree = lockfile.dependency_tree().map_err(|e| {
            AuditError::InnerError(format!(
                "cannot build DepTreeManager, load deptree fails: {e}",
            ))
        })?;

        // TODO: locals not fill
        Ok(Self {
            deptree,
    ...
    fn update_deptree(&mut self) -> Result<(), AuditError> {
        let lockfile = Lockfile::load("Cargo.lock").map_err(|e| {
            AuditError::InnerError(format!("cannot update dep tree, load lock file fails: {e}",))
        })?;

        let dep_tree = lockfile.dependency_tree().map_err(|e| {
            AuditError::InnerError(format!("cannot update dep tree, load dep tree fails: {e}",))
        })?;

        self.deptree = dep_tree;
